...
export const ToolCard = ({ preset }: ToolCardProps) => {
  ...
  return (
    <div
      className="hover:bg-activeCard hover:border-3 group flex flex-col space-y-4 rounded-2xl border border-indigo-200 bg-white p-6 text-gray-900 hover:cursor-pointer hover:text-white dark:border-gray-700 dark:bg-black dark:text-white"
      onClick={handleRedirect}
    >
      <div
        className={clsx(
          "text-primary-dark flex flex-row items-center gap-x-2 dark:text-indigo-200",
          iconHover,
        )}
      >
        {icon}
      ...
      <div className="flex flex-grow flex-col justify-end gap-y-4">
        ...
        <PrimaryButton
          className="font-570 w-fit border-white text-[13px] leading-[13px] backdrop-blur-xl group-hover:border group-hover:bg-white/10 group-hover:text-white dark:group-hover:bg-white dark:group-hover:text-indigo-800"
          label={buttonLabel}

...
import { processProvidedConsolidations } from "pec/server/helpers/process-requests/consolidation";

async function consolidationHandler(_request: Request) {
  const result = await processProvidedConsolidations(MAIN_CHAIN.id);
  if (result.success) {
